...
{
    if (capacity == 0 || index == products.size())
    {
        return 0;
    }
    ...
    else
    {
        ...
        int nibo_andFriend = zeroOneKnapsack_BruteForce(products, capacity - products[index].weight, index + 1);
        take += nibo_andFriend;

        /// Skip the current product
        int notTake = 0;
        int niboNa_andFriend = zeroOneKnapsack_BruteForce(products, capacity, index + 1);
